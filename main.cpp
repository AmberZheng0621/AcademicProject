		...
		std::vector<Vector3f>& p_deformedVertices)
{
	...
	for (unsigned int v = 0; v < p_vertices.size(); v++)
	{
		...
		{
			if (j == 0)
			p_deformedVertices[v] = fromHomog(p_weights[j][v] * p_jointTrans[j] * p_jointTransRestInv[j] * toHomog(p_vertices[v]));

			else
				p_deformedVertices[v] = p_deformedVertices[v]+fromHomog(p_weights[j][v] * p_jointTrans[j] * p_jointTransRestInv[j] * toHomog(p_vertices[v]));
		}
